...
use crate::{FieldBytes, ProjectivePoint, Secp256k1, WideBytes, ORDER, ORDER_HEX};
use core::{
    iter::{Product, Sum},
...
impl Scalar {
    ...
    pub fn from_bytes_mod_order(bytes: &[u8]) -> Self {
        ...
        let mut u8_64 = [0u8; 64];
        u8_64[beg1..].copy_from_slice(&bytes[beg2..]);
        WideScalar::from_bytes(&u8_64).reduce()
    }
