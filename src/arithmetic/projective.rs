...
impl ProjectivePoint {
    ...
    /// Compress this point into a 33-byte array.
    pub fn to33bytes(&self) -> [u8; 33] {
        let ep = self.to_encoded_point(true);
        ep.as_bytes().as_ref().try_into().unwrap()
    }

    /// Decompress a 33-byte array into a point.
    pub fn from33bytes(data: &[u8]) -> Resultat<ProjectivePoint> {
        let ep: _ = EncodedPoint::from_bytes(data).catch(
            "CannotCreatePoint",
            format!("Expected data.len=33, provided data.len={}", data.len()),
        )?;
        ...
        if pp.is_some().into() {
            ...
            throw!(
                "CannotCreatePoint",
                "Failed to create ProjectivePoint from EncodedPoint"
            );
        }
    }
}
...
impl Serialize for ProjectivePoint {
        ...
        S: ser::Serializer,
    {
        ...
        affine.to_encoded_point(true).serialize(serializer)
    }
}
...
impl<'de> Deserialize<'de> for ProjectivePoint {
    ...
    {
        let affine: AffinePoint = EncodedPoint::deserialize(deserializer)?
            .try_into()
